...
def main():
    ...
    print_question(number1,number2,question_type)
    answer = input_answer()
    if answer == -1:
        print_error_message()
    else:
        if is_correct_answer(number1,number2,question_type,answer):
            print_congratulations(question_type)
        else:
            print_correct_answer(number1,number2,question_type)
